...
class Profile(models.Model):
    typeuser =(('student','student'),('Portal Managing', 'Portal Managing'))
    COL=(('GDC TAKHTBHAI','GDC TAKHTBHAI'),('GDC MARDAN','GDC MARDAN')) #change college names
    user =models.OneToOneField(User, on_delete=models.CASCADE,primary_key=True)
    collegename=models.CharField(max_length=29,choices=COL,blank=False)
    phone_regex =RegexValidator(regex=r'^\d{11,11}$', message="Phone number must be entered in the format:Up to 11 digits allowed.")
    contactnumber = models.CharField(validators=[phone_regex], max_length=11, blank=True)
    type_user=models.CharField(max_length=20,default='student',choices=typeuser)
    CB=(('BS ComputerScience',"BS ComputerScience"),('BS Physics',"BS Physics"),('BS English',"BS English"),('BS Political Science',"BS Political Science"))
    Branch=models.CharField(choices=CB,max_length=29,default='BS ComputerScience')
    def __str__(self):
